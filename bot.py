...
async def license_cmd(
    ...
    if not isinstance(target_channel, discord.TextChannel):
        await ctx.send(embed=make_embed(
            "❌ Неверный тип канала",
            "Указанный ID должен принадлежать текстовому каналу.",
            config.COLOR_ERROR,
        ))
        return
    ...
    # Тип 1: все текстовые права КРОМЕ manage_messages
    # Тип 2: все текстовые права ВКЛЮЧАЯ manage_messages
    ...
    for discord_id, user_records in users_data.items():
        ...
        for rec in user_records:
            ...
            line = (
                ...
                f"  Статус: {status}\n"
                f"  Канал: {ch_status}\n"
                f"  Выдана: {fmt_dt(rec['issued_at'])}"
            )
            if rec["revoked"] and rec["revoked_at"]:
                line += f"\n  Отозвана: {fmt_dt(rec['revoked_at'])}"
...
def has_delete_access(member: discord.Member) -> bool:
    return bool({r.id for r in member.roles} & config.DELETE_ALLOWED_ROLE_IDS)
...
_HELP_BLOCKS = [
    ...
    # ── -license ─────────────────────────────────────────────────────────────
    {
        ...
        "description": (
            "Выдаёт лицензию участнику РП и предоставляет права на указанный канал.\n\n"
            "**Типы лицензий:**\n"
            "• `1` — Лицензия **Участника РП** (RPP-XXXXXXX)\n"
            "  Роль: <@&1520321736631914679>\n"
            "  Права на канал: чтение истории и все текстовые права "
            "*кроме* управления сообщениями.\n\n"
            "• `2` — Лицензия **Менеджера РП** (RPM-XXXXXXX)\n"
            "  Роль: <@&1520321736631914678>\n"
            "  Права на канал: чтение истории и все текстовые права "
            "*включая* управление сообщениями.\n\n"
            "**Ограничения:**\n"
            "• Канал должен находиться в специальной категории лицензий.\n"
            "  Если указать канал вне этой категории — команда будет отклонена.\n"
    ...
    # ── -del ─────────────────────────────────────────────────────────────────
    {
        "required_roles": config.DELETE_ALLOWED_ROLE_IDS,
        "emoji": "🗑",
        "name": "-del <количество>",
        "description": (
...
async def help_cmd(ctx: commands.Context):
    ...
    visible_blocks = [
        block for block in _HELP_BLOCKS
        if block["required_roles"] is None
        or bool(author_role_ids & block["required_roles"])
    ]
